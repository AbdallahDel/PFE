...
if (!$user_id) {
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'You must be logged in to accept project requests'
    ]);
    exit();
}
...
try {
    ...
    if ($row_verify['supervisor_id'] != $supervisor_id) {
        throw new Exception("You don't have permission to accept this request");
    }
    
    // Check if the request is still pending
    if ($row_verify['status'] !== 'en attente') {
        throw new Exception("This request cannot be accepted because its current status is '" . $row_verify['status'] . "'");
    }
    
    // Begin transaction
    $conn->begin_transaction();
    
    // Update the status to 'accepted by supervisor'
    $query_update = "
        UPDATE project_choices
        SET status = 'accepted by supervisor'
        WHERE id = ?
    ";
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Project request accepted successfully. Waiting for student confirmation.'
    ]);
    ...
    error_log("Error in acceptProjectRequest.php: " . $e->getMessage());
